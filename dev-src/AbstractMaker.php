...
abstract class AbstractMaker
{
    ...
    public function getBaseClass($save_path, $class_namespace, $class_name, $extend_class = null)
    {
        ...
        if (!file_exists($save_path)) {
            ...
            $parent_methods = (!method_exists($parent_class, 'getMethods')) ? [] : $ref->getParentClass()->getMethods();
    ...
    public function getBaseClassImplements($save_path, $class_namespace, $class_name, $implements_class = null)
    {
        $namespace = new PhpNamespace($class_namespace);

        $implements_class && $namespace->addUse($implements_class);
        if (!file_exists($save_path)) {
            $dir = dirname($save_path);
            if (!is_dir($dir)) {
                @mkdir($dir, 0755, true);
            ...
            $class = $namespace->addClass($class_name);
            if ($implements_class) {
                $class->addImplement($implements_class);
            }
        } else {
            $class = ClassType::from($class_namespace . '\\' . $class_name);
            $namespace->add($class);
