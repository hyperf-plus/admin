...
class Upload
{
    ...
    public function __construct(ContainerInterface $container, RequestInterface $request, ResponseInterface $response)
    {
        ...
        $this->config = $this->container->get(ConfigInterface::class)->get('admin.upload') ?? [];
    }
    ...
    public function image()
    {
        ...
        $fileSize = $this->config['file_size'] ?? 1024 * 1024 * 50;
        ...
        #检测类型
        if (! in_array(strtolower($file->getClientMediaType()), $this->config['media_types'] ?? ['image/gif', 'image/jpeg', 'image/jpg', 'image/png', 'image/pjpeg', 'image/x-png'])) {
    ...
    {
        $file_name = $this->config['save_path'] ?? '/upload';
